...
public static class SecretsPatches
{
    ...
    public static void SetData_Postfix(SecretItemUI __instance)
    {
        try
        {
            ...
            bool haveType = false;
            try { type = RuntimeAccess.Get<SecretType>(__instance, "_type"); haveType = true; } catch { }
            ...
            if (data == null) { try { data = RuntimeAccess.Get<SecretData>(__instance, "_data"); } catch { } }

            bool achieved = false;
            try { achieved = RuntimeAccess.Get<bool>(__instance, "_hasAchieved"); } catch { }
    ...
    {
        string type = "?";
        try { type = RuntimeAccess.Get<SecretType>(item, "_type").ToString(); } catch { }
        ...
        try
        {
            Image r = RuntimeAccess.Get<Image>(item, "_Reward");
    ...
    private static GameObject RewardImage(SecretItemUI item)
    {
        try
        {
            Image img = RuntimeAccess.Get<Image>(item, "_Reward");
