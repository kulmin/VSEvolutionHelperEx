...
#if VSEH_MONO
using WeaponTypeArray = VampireSurvivors.Data.WeaponType[];
#else
using WeaponTypeArray = Il2CppInterop.Runtime.InteropTypes.Arrays.Il2CppStructArray<VampireSurvivors.Data.WeaponType>;
#endif
...
public class ItemTooltipsMod
{
    ...
    private static void ApplyPatches()
    {
        try
        {
            ...
            MethodInfo method = typeFromHandle.GetMethod("OnShowStart",
    ...
    private static void TryEarlyCaching()
    {
        ...
        try
        {
            ...
            var source = UnityObjectQuery.FindAll<MonoBehaviour>();
    ...
    private static void ScanForIcons()
    {
        ...
        foreach (Transform activeUIContainer in activeUIContainers)
        {
            ...
            var componentsInChildren = ((Component)activeUIContainer).GetComponentsInChildren<Image>(false);
            ...
            foreach (Image item in componentsInChildren)
            {
                ...
                if (weaponType.HasValue && ((object)weaponType.Value).ToString() == "DEFANG")
                {
                    weaponType = null;
                }
                if (itemType.HasValue && ((object)itemType.Value).ToString() == "DEFANG")
                {
                    itemType = null;
                }
    ...
    private static void AddHoverToIcon(TrackedIcon tracked)
    {
        ...
        if (!((Object)(object)component != (Object)null))
        {
            ...
            ((UnityEvent<BaseEventData>)(object)val2.callback).AddListener(RuntimeEvents.Wrap(delegate
            {
            ...
            ((UnityEvent<BaseEventData>)(object)val3.callback).AddListener(RuntimeEvents.Wrap(delegate
            {
    ...
    internal static void AdoptWeaponSelectionView(Transform cell)
    {
        ...
        // Log the hierarchy once so an inactive cached view can be identified.
    ...
    /// <summary>
    /// Fallback for cells bound before the targeted weapon-selector patches run.
    /// </summary>
    private static void ScanWeaponSelectionView(GameObject viewGo)
    {
        ...
        Plugin.Log.LogInfo($"[WeaponSelect] Fallback scan: {registered}/{cells.Length} cells");
    }
    ...
    private static int ScanChildrenForTypes(Transform parent, int depth, int maxDepth)
    {
        ...
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            WeaponType? weaponType = null;
            ItemType? itemType = null;
            var components = ((Component)child).GetComponents<Component>();
            ...
            while ((bool)method.Invoke(obj, null))
            {
                ...
                for (int i = 0; i < weaponDataList.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (property3 != null && property3.GetValue(val2) is WeaponTypeArray val3)
                        {
                            for (int j = 0; j < val3.Length; j++)
                            {
                                WeaponType val4 = val3[j];
                                int num = (int)val4;
    ...
    private static void TryFindGameSession()
    {
        try
        {
            ...
            try
            {
                ...
                if ((Object)(object)val != (Object)null)
                {
                    var components = val.GetComponents<Component>();
                    foreach (Component item in components)
                    {
                        if ((Object)(object)item == (Object)null)
                        {
            ...
            {
                var componentsInChildren = item3.GetComponentsInChildren<Component>(true);
    ...
    private static void TryCacheDataManagerStatic()
    {
        ...
        try
        {
            ...
            var val = UnityObjectQuery.FindAll<MonoBehaviour>();
            for (int num3 = 0; num3 < val.Length && num3 < 200; num3++)
            {
    ...
    public static void CacheDataManager(object dataManager)
    {
        ...
        try
        {
            ...
            List<string> list = (from kv in spriteToWeaponType
                                 where ((object)kv.Value).ToString() == "DEFANG"
    ...
    {
        formulaIcons.Clear();
        var componentsInChildren = popup.GetComponentsInChildren<EventTrigger>(false);
    ...
    private static void SetEquipmentHighlight(int index)
    {
        ...
        float num = 48f;
        var componentsInChildren = val.GetComponentsInChildren<Image>(false);
        foreach (Image item in componentsInChildren)
        {
    ...
    public static void ShowItemPopup(Transform anchor, WeaponType? weaponType, ItemType? itemType)
    {
        ...
        if ((weaponType.HasValue && ((object)weaponType.Value).ToString() == "DEFANG") || (itemType.HasValue && ((object)itemType.Value).ToString() == "DEFANG"))
        {
            return;
        }
        ...
        if (!IsGamePaused())
        {
            ...
            {
                // Character selection uses a cascading menu popup anchored to the clicked icon.
    ...
    private static GameObject CreatePopup(Transform parent, WeaponType? weaponType, ItemType? itemType)
    {
        ...
        if ((Object)(object)font != (Object)null)
        {
            ...
            ((TMP_Text)val10).textWrappingMode = TextWrappingModes.Normal;
            ...
            if (!string.IsNullOrEmpty(text2))
            {
                ...
                ((TMP_Text)val16).textWrappingMode = TextWrappingModes.Normal;
            ...
            {
                string value = ((object)itemType.Value).ToString();
                bool flag = false;
    ...
    private static List<PassiveRequirement> CollectPassiveRequirements(WeaponTypeArray evoSynergy, HashSet<int> requiresMaxTypes = null)
    {
    ...
    private unsafe static bool PlayerOwnsWeapon(WeaponType weaponType)
    {
        ...
        try
        {
            ...
            string text = ((object)(*(WeaponType*)(&weaponType))).ToString();
            for (int i = 0; i < num; i++)
            {
                object value4 = property5.GetValue(value3, new object[1] { i });
                if (value4 == null)
    ...
    private unsafe static bool PlayerOwnsItem(ItemType itemType)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                {
                    object value5 = property6.GetValue(value4);
                    if (value5 != null && value5.ToString() == ((object)(*(ItemType*)(&itemType))).ToString())
                    {
                        return true;
                    }
                }
    ...
    private unsafe static bool IsItemBanned(ItemType itemType)
    {
        try
        {
            ...
            foreach (string name in array3)
            {
                ...
                int num = (int)property3.GetValue(value);
                string text = ((object)(*(ItemType*)(&itemType))).ToString();
                for (int j = 0; j < num; j++)
                {
        ...
        try
        {
            ...
            while ((bool)method.Invoke(obj, null))
            {
                ...
                for (int i = 0; i < weaponDataList.Count; i++)
                {
                    ...
                    if (property3 != null && property3.GetValue(val) is WeaponTypeArray val2)
                    {
                        for (int j = 0; j < val2.Length; j++)
                        {
                            if (val2[j] == passiveType)
                            {
                                num++;
                                break;
                            }
    ...
    private unsafe static float AddEvolvedFromSection(Transform parent, TMP_FontAsset font, WeaponType evolvedType, float yOffset, float maxWidth)
    {
        ...
        string text = ((object)(*(WeaponType*)(&evolvedType))).ToString();
                ...
                for (int i = 0; i < weaponDataList.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(propertyValue) && propertyValue == text)
                        {
                            ...
                            if (property3 != null)
                            {
                                WeaponTypeArray evoSynergy = property3.GetValue(val2) as WeaponTypeArray;
    ...
    private unsafe static float AddPassiveEvolutionSection(Transform parent, TMP_FontAsset font, WeaponType passiveType, float yOffset, float maxWidth)
    {
        ...
        try
        {
            ...
            while ((bool)method.Invoke(obj, null))
            {
                ...
                for (int i = 0; i < weaponDataList.Count; i++)
                {
                    ...
                    {
                        PropertyInfo property3 = ((object)val2).GetType().GetProperty("evoSynergy");
                        if (property3 == null || !(property3.GetValue(val2) is WeaponTypeArray val3) || val3.Length == 0)
                        {
                            continue;
                        }
                        string propertyValue = GetPropertyValue<string>(val2, "evoInto");
        ...
        try
        {
            ...
            if (weaponData2 != null)
            {
                ...
                if (!string.IsNullOrEmpty(propertyValue2) && ((object)weaponData2).GetType().GetProperty("evoSynergy")?.GetValue(weaponData2) is WeaponTypeArray evoSynergy && GameData.TryParseWeaponType(propertyValue2, out WeaponType result2))
                {
                ...
                while ((bool)method2.Invoke(obj5, null))
                {
                    ...
                    for (int l = 0; l < weaponDataList2.Count; l++)
                    {
                        ...
                        try
                        {
                            ...
                            item3.EvolvedName = ((weaponData4 != null) ? GetLocalizedWeaponName(weaponData4, val5) : ((object)(*(WeaponType*)(&val5))).ToString());
                            bool flag6 = false;
                ...
                for (int i = 0; i < weaponDataList.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(propertyValue))
                        {
                            ...
                            for (int j = 0; j < val3.Length; j++)
                            {
                                text = text + ((object)val3[j]).ToString() + " ";
                            }
                        }
                        string text2 = ((object)(*(ItemType*)(&itemType))).ToString();
                        bool flag = false;
                        for (int k = 0; k < val3.Length; k++)
                        {
                            if (((object)val3[k]).ToString() == text2)
                            {
                                flag = true;
                                break;
                            }
    ...
    private static void ShowArcanaPopup(Transform anchor, ArcanaType arcanaType)
    {
        if (!IsGamePaused())
        {
            // Menu arcanas use the collection detail panel; character selection cascades it.
    ...
    private static void HideTopPopup()
    {
        ...
        if ((Object)(object)val != (Object)null)
        {
            var componentsInChildren = val.GetComponentsInChildren<EventTrigger>(false);
            foreach (EventTrigger item in componentsInChildren)
            {
    ...
    private static void EnableCollectionPanelInteraction(GameObject popup)
    {
        ...
        try
        {
            // Interactive popups require a GraphicRaycaster on an owning canvas.
            ...
            enter.callback.AddListener(RuntimeEvents.Wrap(delegate
            {
            ...
            exit.callback.AddListener(RuntimeEvents.Wrap(delegate
            {
    ...
    private static void HideCollectionPopup()
    {
        if ((Object)(object)collectionPopup != (Object)null)
        {
            var componentsInChildren = collectionPopup.GetComponentsInChildren<EventTrigger>(false);
            foreach (EventTrigger item in componentsInChildren)
            {
                formulaIconData.Remove(((Object)((Component)item).gameObject).GetInstanceID());
            }
    ...
    {
        if (((object)(*(WeaponType*)(&type))).ToString() == "DEFANG")
        {
            return;
        }
        ...
        if (((object)(*(WeaponType*)(&type))).ToString() == "VOID")
        {
            return;
        }
    ...
    {
        if (((object)(*(ItemType*)(&type))).ToString() == "DEFANG")
        {
            return;
        }
        ...
        if (((object)(*(ItemType*)(&type))).ToString() == "VOID")
        {
            return;
        }
        if (!IsUnderGameUI(go.transform))
    ...
    private static void ShowCharacterPopup(MapIconInfo info)
    {
        ...
        try { CharacterSelectPatches.TryGetTooltipData(info.Go, out data); }
        catch (Exception ex)
        {
        ...
        float size, Color color, bool bold, float x, float y, float w, float h, TextAlignmentOptions align)
    {
        ...
        ((TMP_Text)tmp).textWrappingMode = TextWrappingModes.Normal;
    ...
    private static float MeasureTmpPreferredWidth(TMP_FontAsset font, string text, float fontSize, bool bold)
    {
        ...
        try
        {
            ...
            ((TMP_Text)tmp).fontStyle = bold ? (FontStyles)1 : (FontStyles)0;
            ((TMP_Text)tmp).textWrappingMode = TextWrappingModes.NoWrap;
            ((TMP_Text)tmp).ForceMeshUpdate(true, true);
    ...
    private static Transform FindMapPopupParent(Transform anchor)
    {
        ...
        try
        {
            var map = UnityObjectQuery.FindAny<MapManager>();
        ...
        List<GameData.IconRow> rows, string sectionHeader)
    {
        ...
        ((TMP_Text)titleTmp).textWrappingMode = TextWrappingModes.Normal;
        ...
        if (!string.IsNullOrEmpty(description))
        {
            ...
            ((TMP_Text)dt).textWrappingMode = TextWrappingModes.Normal;
        ...
        float iconSize, float nameX, float nameW, float y, int index)
    {
        ...
        if ((Object)(object)tmp != (Object)null)
            // The next row starts after the full rendered height, including wrapped price ladders.
    ...
    ///
    /// A container scaled to zero cannot draw even when its rect and alpha are valid. Force the
    /// canvas update before reading <c>lossyScale</c> so the decision uses current layout state.
    /// </summary>
    ...
    private static Canvas TopmostCanvas(GameObject self)
    {
        ...
        try
        {
            Canvas[] all = UnityObjectQuery.FindAll<Canvas>();
        ...
        Vector2? offset, Vector2? pivot)
    {
        try
        {
            ...
            // Recheck after parenting because building the popup can change the canvas layout.
    ...
    private static GameObject FindIconInUI(GameObject parent)
    {
        ...
        var componentsInChildren = parent.GetComponentsInChildren<Image>(false);
        foreach (Image item in componentsInChildren)
        {
    ...
    private static void AddHoverToGameObject(GameObject go, WeaponType? weaponType, ItemType? itemType, bool useClick = false, Transform popupAnchor = null)
    {
        ...
        if (useClick)
        {
            ...
            ((UnityEvent<BaseEventData>)(object)click.callback).AddListener(RuntimeEvents.Wrap(delegate
            {
        ...
        else
        {
            ...
            ((UnityEvent<BaseEventData>)(object)val4.callback).AddListener(RuntimeEvents.Wrap(delegate
            {
    ...
    private unsafe static string GetLocalizedPowerUpName(object data, ItemType type)
    {
        if (data == null)
        {
            return ((object)(*(ItemType*)(&type))).ToString();
        }
        try
        {
    ...
    {
        TextMeshProUGUI obj = UnityObjectQuery.FindAny<TextMeshProUGUI>();
            ...
            if (!arcanaDebugLogged)
            {
                ...
                foreach (object item in allActiveArcanaTypesInternal)
                {
                    ...
                    if (property2 != null)
                    {
                        ...
                        if (value2 != null)
                        {
                            ...
                            for (int num2 = 0; num2 < num; num2++)
                            {
                                ...
                                try
                                {
                                    int num3 = GetArcanaTypeInt(value3);
                                    object obj2 = (Enum.IsDefined(typeof(WeaponType), num3) ? ((object)(WeaponType)num3).ToString() : "UNDEFINED");
                    ...
                    for (int num5 = 0; num5 < num4; num5++)
                    {
                        ...
                        try
                        {
                            int num6 = GetArcanaTypeInt(value5);
                            object obj4 = (Enum.IsDefined(typeof(ItemType), num6) ? ((object)(ItemType)num6).ToString() : "UNDEFINED");
    ...
    private static Sprite LoadArcanaSprite(string textureName, string frameName)
    {
        ...
        var val2 = Resources.FindObjectsOfTypeAll<Sprite>();
    ...
    private static void ScanArcanaUI(int arcanaTypeInt, string arcanaName)
    {
        ...
        try
        {
            var val = UnityObjectQuery.FindAll<TextMeshProUGUI>();
            Transform val2 = null;
            ...
            foreach (TextMeshProUGUI item in val)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    var componentsInChildren = ((Component)parent).GetComponentsInChildren<Image>();
            ...
            var componentsInChildren2 = ((Component)val2).GetComponentsInChildren<Image>();
