...
public static class AscensionPatches
{
    ...
    public static void Tick()
    {
        ...
        try
        {
            ...
            try { buttons = UnityObjectQuery.FindAll<AscensionButton>(true); } catch { }
            try { panels = UnityObjectQuery.FindAll<AscensionPanel>(true); } catch { }
    ...
    {
        Vector2 offset = AscensionOffset();
        Register(RuntimeAccess.Get<AdjustValuePanel>(panel, "_LuckPanel"), PowerUpType.LUCK, panel, offset);
        Register(RuntimeAccess.Get<AdjustValuePanel>(panel, "_GrowthPanel"), PowerUpType.GROWTH, panel, offset);
        Register(RuntimeAccess.Get<AdjustValuePanel>(panel, "_GreedPanel"), PowerUpType.GREED, panel, offset);
        Register(RuntimeAccess.Get<AdjustValuePanel>(panel, "_CursePanel"), PowerUpType.CURSE, panel, offset);
    }
    ...
    private static Vector2 AscensionOffset()
    {
        try
        {
            var page = UnityObjectQuery.FindAny<SelectAdventuresPage>();
    ...
    private static List<GameData.IconRow> BuildRows(AdjustValuePanel control, AscensionPanel panel)
    {
        ...
        try { assigned = RuntimeAccess.Get<int>(control, "_pointsAssigned"); } catch { }
        try { increment = RuntimeAccess.Get<float>(control, "_IncrementAmount"); } catch { }
        try { suffix = RuntimeAccess.Get<string>(control, "_Suffix"); } catch { }
        try { displayed = RuntimeAccess.Get<TMPro.TextMeshProUGUI>(control, "_ValueText").text; } catch { }
        try { available = RuntimeAccess.Get<int>(panel, "_completionCount") - RuntimeAccess.Get<int>(panel, "_currentSpend"); } catch { }
    ...
    private static Sprite Sprite(AdjustValuePanel control, PowerUpType type)
    {
        try
        {
            var image = RuntimeAccess.Get<UnityEngine.UI.Image>(control, "_Icon");
