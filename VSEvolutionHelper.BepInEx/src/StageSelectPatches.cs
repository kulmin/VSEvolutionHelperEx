...
#if VSEH_MONO
using RuntimeGameObjectList = System.Collections.Generic.List<UnityEngine.GameObject>;
using RuntimeItemTypeList = System.Collections.Generic.List<VampireSurvivors.Data.ItemType>;
#else
using RuntimeGameObjectList = Il2CppSystem.Collections.Generic.List<UnityEngine.GameObject>;
using RuntimeItemTypeList = Il2CppSystem.Collections.Generic.List<VampireSurvivors.Data.ItemType>;
#endif
...
public static class StageSelectPatches
{
    ...
    public static void SetRelics_Postfix(RelicPanel __instance, StageData stage, StageType stageType)
    {
        try
        {
            ...
            RuntimeGameObjectList spawned = null;
            RuntimeItemTypeList types = null;
            try { spawned = RuntimeAccess.Get<RuntimeGameObjectList>(__instance, "_spawned"); } catch { }
            try { types = RuntimeAccess.Get<RuntimeItemTypeList>(__instance, "_spawnedType"); } catch { }
    ...
    private static System.Collections.Generic.List<ItemType> FlattenRelics(StageData stage)
    {
        ...
        void addList(RuntimeItemTypeList src)
        {
            if (src == null) return;
