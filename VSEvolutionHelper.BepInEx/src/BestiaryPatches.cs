...
/// <summary>
/// Adds enemy stats and resistance tooltips after the Bestiary finishes populating. Do not patch
/// <c>EnemyItemUI.SetData</c>; its nullable DLC argument is not safe across the IL2CPP Harmony
/// trampoline.
/// </summary>
public static class BestiaryPatches
{
    ...
    /// <summary>
    /// Placement in 1920x1200 safe-area coordinates, pinned at the top right.
    /// </summary>
    ...
    private static void ScanAndRegister()
    {
        ...
        try
        {
            ...
            try { items = UnityObjectQuery.FindAll<EnemyItemUI>(true); }
            catch { try { items = UnityObjectQuery.FindAll<EnemyItemUI>(); } catch { } }
    ...
    private static void RegisterOne(EnemyItemUI __instance)
    {
        try
        {
            ...
            bool killed = false;
            try { killed = RuntimeAccess.Get<bool>(__instance, "_hasKilled"); } catch { }
            if (!killed && !Plugin.BestiarySpoilers) return;

            VampireSurvivors.Data.Enemies.EnemyData data = null;
            try { data = RuntimeAccess.Get<VampireSurvivors.Data.Enemies.EnemyData>(__instance, "_data"); } catch { }
            if (data == null) return;

            string enemyId = null;
            try { enemyId = RuntimeAccess.Get<VampireSurvivors.Data.EnemyType>(__instance, "_type").ToString(); } catch { }

            VampireSurvivors.Data.EnemyType? type = null;
            try { type = RuntimeAccess.Get<VampireSurvivors.Data.EnemyType>(__instance, "_type"); } catch { }
    ...
    public static void PageInfoPanel_Postfix(VampireSurvivors.UI.BestiaryPage __instance)
    {
        try
        {
            ...
            string title = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_Title"));
            if (string.IsNullOrWhiteSpace(title)) return;

            var panel = new GameData.EnemyPanel
            {
                Hp = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_HP")),
                Power = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_Power")),
                Speed = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_Speed")),
                Resistances = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_Resistances")),
                Skills = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_Skills")),
                FoundIn = Text(RuntimeAccess.Get<TMPro.TextMeshProUGUI>(__instance, "_FoundIn")),
            };
    ...
    private static string ResolveName(EnemyItemUI item)
    {
        ...
        try
        {
            var label = RuntimeAccess.Get<TMPro.TextMeshProUGUI>(item, "_Name");
        ...
        try
        {
            string b = GameData.GetEnemyName(RuntimeAccess.Get<VampireSurvivors.Data.EnemyType>(item, "_type").ToString());
            if (!string.IsNullOrWhiteSpace(b) && !IsPlaceholder(b)) return b;
        }
        catch { }
        try
        {
            string id = RuntimeAccess.Get<VampireSurvivors.Data.EnemyType>(item, "_type").ToString();
    ...
    private static void CapturePanelSprite(VampireSurvivors.UI.BestiaryPage page)
    {
        try
        {
            ...
            if (added > 0) GameData.BumpSpriteGeneration();
                ...
                Plugin.Dbg($"[Bestiary] no new portraits among {images.Length} images");
        }
        catch (Exception ex)
        {
    ...
    {
        try { return RuntimeAccess.Get<VampireSurvivors.Data.EnemyType>(item, "_type").ToString(); } catch { return "?"; }
    }
