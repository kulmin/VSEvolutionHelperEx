...
public static class CollectionSelectPatches
{
    ...
    public static void ScanAndRegister()
    {
        ...
        try
        {
            ...
            try { uis = UnityObjectQuery.FindAll<CollectionItemUI>(); }
            catch
            {
                try { uis = UnityObjectQuery.FindAll<CollectionItemUI>(); } catch { }
            }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(its) && its != "0" && Enum.IsDefined(typeof(ItemType), it))
            {
                ...
                if (IsVisuallyLocked(ui))
                {
                    ...
                    try
                    {
                        var lockedIcon = RuntimeAccess.Get<Image>(ui, "LockedIcon");
                        if ((Object)(object)lockedIcon != (Object)null)
                            spr = lockedIcon.sprite;
                    }
                    catch { }
    ...
    private static bool IsVisuallyLocked(CollectionItemUI ui)
    {
        try
        {
            var locked = RuntimeAccess.Get<Image>(ui, "LockedIcon");
