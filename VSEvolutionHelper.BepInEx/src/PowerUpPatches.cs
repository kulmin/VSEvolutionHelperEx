...
public static class PowerUpPatches
{
    ...
    public static void SetData_Postfix(PowerUpItemUI __instance)
    {
        try
        {
            ...
            int maxLevel = 0;
            try { maxLevel = RuntimeAccess.Get<int>(__instance, "_maxRank"); }
            catch { }
        ...
        PowerUpsPage page, PowerUpType type, int maxLevel, out string description)
    {
        ...
        try
        {
            var stats = RuntimeAccess.Get<VampireSurvivors.PlayerStats>(page, "_playerStats");
            if (stats != null)
            {
    ...
    private static void LogPricing(PowerUpItemUI ui, PowerUpType type, int maxLevel)
    {
        try
        {
            int level = -1;
            float next = -1f;
            try { level = RuntimeAccess.Get<int>(ui, "_currentLevel"); } catch { }
            double markup = -1d;
            double baseMarkup = -1d;
            int owned = -1, ownedLevel = -1;
            try
            {
                var stats = RuntimeAccess.Get<VampireSurvivors.PlayerStats>(ui._page, "_playerStats");
                if (stats != null)
                {
    ...
    private static string ResolveTitle(PowerUpItemUI ui, PowerUpType type)
    {
        ...
        try
        {
            var loc = RuntimeAccess.Get<I2.Loc.Localize>(ui, "Title");
    ...
    private static Sprite ResolveSprite(PowerUpItemUI ui, PowerUpType type)
    {
        try
        {
            var img = RuntimeAccess.Get<UnityEngine.UI.Image>(ui, "Icon");
